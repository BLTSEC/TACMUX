"""Primary engagement cockpit screen."""
...
from ..hooks import clipboard_copy
from ..model import (
    AccessRecord,
    Activity,
    ActivityResult,
...
from ..store import EngagementRecord, safe_filename
from ..ui import plain
from .picker import EngagementPickerScreen
...
class MainScreen(Screen):
    ...
    operator_commands: ClassVar[list[tuple[str, str, str]]] = [
        ...
        (
            "Edit engagement details and authorization",
            "edit_engagement",
            "Update the assessment identity, authorization, window, and logging policy",
        ),
        (
    ...
    def __init__(self, record: EngagementRecord, initial_target_id: str = ""):
        ...
        self._filter_return_focus = "target-table"
    ...
    def operator_command_available(self, action: str) -> bool:
        if action == "attack_path" and not self._has_attack_path_steps():
            return False
        return not (
            self.engagement.status == EngagementStatus.CLOSED
    ...
    def _has_attack_path_steps(self) -> bool:
        return bool(
            self.engagement.access
            or any(
                item.result == ActivityResult.CONFIRMED
                for item in self.engagement.activities
            )
            or any(
                item.state in {FindingState.CONFIRMED, FindingState.CLOSED}
    ...
    def _status_line(self, jobs: list[dict]) -> Text:
        ...
        if self.engagement.status != EngagementStatus.CLOSED:
            if state == "outside":
                metrics.append((" outside authorized window", theme.warning))
            ...
            elif self.engagement.authorization.window_start:
                metrics.append(
                    (
                        f" active since {self.engagement.authorization.window_start}",
                        theme.primary,
                    )
                )
            else:
                metrics.append(
                    (" authorization window not configured", theme.warning)
                )
    ...
    def _populate_panes(self, panes: set[str], jobs: list[dict]) -> None:
        ...
        if "scope" in panes:
            self.query_one(ScopeDiscoveryPane).populate(
                self.engagement,
                jobs,
                query if active == "scope" else "",
            )
        if "records" in panes:
            kind_filter = self.query_one("#records-kind", Select).value
    ...
    def action_filter(self) -> None:
        field = self.query_one("#target-filter", Input)
        active = self.query_one("#workspace-tabs", TabbedContent).active
        if active not in {"targets", "scope", "records", "documents"}:
            self.app.notify(
                "Find is available on Targets, Scope, Records, and Documents"
            )
            return
        focused = self.app.focused
        if isinstance(focused, DataTable) and focused.id:
            self._filter_return_focus = focused.id
    ...
    def action_close_filter(self) -> None:
        ...
        default_focus = {
            ...
            "records": "records-table",
            "documents": "documents-table",
        }.get(active, "target-table")
        focus_id = self._filter_return_focus
        if focus_id not in {
            "target-table",
            "scope-table",
            "jobs-table",
        ...
        self.query_one(f"#{focus_id}", DataTable).focus()
    ...
    def filter_changed(self, event: Input.Changed) -> None:
        ...
        if active == "records":
            ...
            self.query_one(ScopeDiscoveryPane).populate(
                self.engagement,
                self.app.jobs.list(self.record.root),
                event.value,
            )
        elif active == "documents":
    ...
    @on(Select.Changed, "#records-kind")
    def records_kind_changed(self, event: Select.Changed) -> None:
        query = self.query_one("#target-filter", Input).value
        self.query_one(RecordsPane).populate(
            self.engagement,
            query
            if self.query_one("#workspace-tabs", TabbedContent).active == "records"
            else "",
            root=self.record.root,
            kind_filter=str(event.value),
        )
    ...
    def action_help(self) -> None:
        self.app.push_screen(
            MessageModal(
                ...
                "  /      Find in Targets, Scope, Records, or Documents\n"
    ...
    def action_edit_engagement(self) -> None:
        try:
            self._require_active()
            self.app.push_screen(
                EngagementDetailsForm(self.engagement),
                self._save_engagement_details,
            )
        except TacmuxError as exc:
            self.app.show_error(str(exc))

    def _save_engagement_details(self, value: dict | None) -> None:
        if value is None:
            return
        try:
            ...
            self.after_mutation("situation", "documents")
            self.app.notify("Engagement details and authorization updated")
        except (TacmuxError, OSError, ValueError) as exc:
            self.app.show_error(str(exc))
    ...
    def action_actions(self) -> None:
        ...
        if active == "scope":
            ...
            if self.engagement.status != EngagementStatus.CLOSED:
                ...
                if self._has_attack_path_steps():
    ...
    def target_actions(self) -> None:
        ...
        if self.engagement.status == EngagementStatus.CLOSED:
            ...
            if target.primary_endpoint:
                actions.insert(1, ("copy_endpoint", "Copy primary endpoint"))
        ...
        if target.primary_endpoint:
            actions.append(("copy_endpoint", "Copy primary endpoint"))
        if running:
    ...
    def _target_action(self, action: str | None) -> None:
        ...
        dispatch = {
            ...
            "copy_endpoint": self.copy_primary_endpoint,
    ...
    def copy_primary_endpoint(self) -> None:
        target = self.selected_target()
        if not target.primary_endpoint:
            self.app.show_error("The selected target has no primary endpoint")
            return
        try:
            if clipboard_copy(self.app.tmux, target.primary_endpoint.encode()) != 0:
                raise TacmuxError("clipboard command failed")
            self.app.notify(f"Copied primary endpoint: {target.primary_endpoint}")
        except (TacmuxError, OSError) as exc:
            self.app.show_error(str(exc))
    ...
    def edit_target_notes(self) -> None:
        target = self.selected_target()
        self.app.edit_file(
            self.record.root / "targets" / target.directory / "NOTES.md",
            allowed_root=self.record.root,
        )
        self.query_one(DocumentsPane).populate(self.record)
    ...
    def _create_finding(self, value: dict | None) -> None:
        ...
        try:
            ...
            self.app.edit_file(
                self.record.root / finding.document,
                allowed_root=self.record.root,
            )
            self.after_mutation("records")
        except (TacmuxError, OSError) as exc:
    ...
    def records_actions(self) -> None:
        if self.engagement.status == EngagementStatus.CLOSED:
            ...
            return
        selected = self.query_one(RecordsPane).selected_record()
        actions = [("activity", "Record activity")]
        if self.engagement.targets:
            actions.extend(
                [
                    ("finding", "Create finding"),
                    ("cleanup", "Record cleanup item"),
        ...
        if self._has_attack_path_steps():
            actions.append(("attack_path", "Build confirmed attack path"))
        if selected:
    ...
    def _records_action(
        ...
        if not action:
            return
        if action in {"activity", "finding", "cleanup", "attack_path"}:
            if action == "cleanup":
                self.record_cleanup()
                return
            if action == "finding":
    ...
    def edit_selected_document(self) -> None:
        ...
        if not editable:
            ...
            return
        self.app.edit_file(path, allowed_root=self.record.root)
        ...
        if action == "export":
            ...
            try:
                relative = path.resolve(strict=True).relative_to(
                    self.record.root.resolve(strict=True)
                )
                if clipboard_copy(self.app.tmux, str(relative).encode()) != 0:
                    raise TacmuxError("clipboard command failed")
                self.app.notify(f"Copied document path: {relative}")
            except (TacmuxError, OSError, ValueError) as exc:
                self.app.show_error(str(exc))
    ...
    def action_restore(self) -> None:
        try:
            ...
            self.app.show_error(str(exc))
            return
        self.restore_options = cockpit_archives(
            self.app.settings.archive_dir, self.engagement.id
        )
